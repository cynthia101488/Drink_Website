...
public class OrderDetailHelper {
    ...
    public JSONArray createByList(long id_Order, List<OrderDetail> orderdetail) {
        ...
        for(int i=0 ; i < orderdetail.size() ; i++) {
            ...
            try {
                ...
                /** SQL指令 */
                String sql = "INSERT INTO `sa`.`orderdetails`(`id_Order`, `id_Product`, `specification_Product`, `quantity_Product`, `price_Product`,"
    ...
    public JSONObject getByEId(String orderdetail_id) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `sa`.`orderdetails` WHERE `id` = ?";
            
            /** 將參數回填至SQL指令當中，若無則不用只需要執行 prepareStatement */
            pres = conn.prepareStatement(sql);
            pres.setString(1, orderdetail_id);
            /** 執行查詢之SQL指令並記錄其回傳之資料 */
            ...
            while(rs.next()) {
                ...
                String id_Cup=rs.getString("id_Cup");
               
                
                /** 將每一筆商品資料產生一名新Product物件 */
                od = new OrderDetail(id, id_Order, id_Product, specification_Product, quantity_Product, id_Cup);
                /** 取出該項商品之資料並封裝至 JSONsonArray 內 */
                //data = o.getOrderAllInfo();
                //data=o.getOrderData();
                jsa.put(od.getOrderAllInfo());
    ...
    public JSONObject getById(String order_id) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `sa`.`orderdetails` WHERE `id_Order` = ?";
    ...
    public JSONObject getByMId(String member_id) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `sa`.`orders` WHERE `id_Member` = ?";
            ...
            while(rs.next()) {
                ...
                Timestamp createOforder = rs.getTimestamp("dateOforder");     
                
                
                /** 將每一筆商品資料產生一名新Product物件 */
                o = new Order(id,  id_Member, phoneNumber_Member, createOforder, stateOforder, sn_Coupon);
                /** 取出該項商品之資料並封裝至 JSONsonArray 內 */
                //data = o.getOrderAllInfo();
                //data=o.getOrderData();
    ...
    public JSONObject deleteByID(int id) {
        ...
        try {
            ...
            /** SQL指令 */
            String sql = "DELETE FROM `sa`.`orderdetails` WHERE `id_Order` = ? LIMIT 1";
    ...
    public JSONObject getAll() {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `sa`.`orderdetails`";
    ...
    public JSONObject update(OrderDetail od) {
        /** 紀錄回傳之資料 */
        JSONArray jsa = new JSONArray();
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Select `quantity_Cup` , `quantity_Product` FROM `sa`.`orderdetails`   WHERE `id` = ?";
            
            int id=od.getId();
            int quantity = od.getQuantity_Product();
            pres = conn.prepareStatement(sql);
            pres.setInt(1, id);
            ...
            while(rs.next()) {
                /** 每執行一次迴圈表示有一筆資料 */
                 ...
                 quantity_Cup = rs.getInt("quantity_Cup");
            }
            
            
            if(quantity_Cup != quantity_Product) {
            	 sql = "Update `sa`.`orderdetails` SET `quantity_Product` = ?  WHERE `id` = ?";
            	...
            	sql = "Update `sa`.`orderdetails` SET `quantity_Product` = ? , `quantity_Cup` = ? WHERE `id` = ?";
            	
                /** 將參數回填至SQL指令當中 */
                pres = conn.prepareStatement(sql);
                pres.setInt(1, quantity);
                pres.setInt(2, quantity);
                pres.setInt(3, id);
